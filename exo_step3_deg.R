...
##不同risk group 差异分析
load("~/TCGA/Copper_death/Copper_all.rds")
head(sub)

load("~/TCGA/exosome_ccRCC/tcgasub.rds")
sub_exo <- data
sub_exo$Sample <- rownames(sub_exo)
sub_exo$Cluster <- ifelse(sub_exo$group=="High","C1","C2")
sub <- sub_exo
##转接之前的铜死亡项目alldata进行分组分析
...
rownames(sub) <- str_sub(rownames(sub),1,15)
...
class(sub)
colnames(sub)
meta <- sub[,c("Sample","Cluster","EVENT","OS")]
head(meta)
head(sub)
...
setwd("/home/data/vip39/TCGA/Copper_death/complexheatmap/")
...
Subtype <- Subtype[-2]
mygene_data <- KIRC_mRNA_fpkm[RNA_gene,rownames(Subtype)]
...
pheatmap(cellheight = 15, cellwidth = 1,
         ...
         gaps_col = c(346,516), # 根据自己的数据设置空白间隔的位置
...
if(!file.exists(paste0("CS2_CS1","dd.Rdata"))){
  dds <- DESeqDataSetFromMatrix(
    ...
    design = ~ condition)
  dds <- DESeq(dds)
  save(dds,file = paste0("CS2_CS1","_dd.Rdata"))
}
load(paste0("CS2_CS1","_dd.Rdata"))
...
up <- rownames(DEG)[DEG$change=="UP"]#差异上调
down <- rownames(DEG)[DEG$change=="DOWN"]#差异下调
...
ggplot(sortdf, aes(ID, score, fill = group)) + geom_bar(stat = 'identity') + 
  ...
  xlab("") +ylab("t value of GSVA score, C2 versus C1")+
  theme_bw() + #去除背景色
  theme(panel.grid =element_blank()) + #去除网格线
  theme(panel.border = element_rect(size = 0.6)) + #边框粗细
  theme(axis.line.y = element_blank(), axis.ticks.y = element_blank(), axis.text.y = element_blank()) #去除y轴
...
hmdat <- read.csv("~/huitu/FigureYa230immunelandscape/easy_input.csv",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
type <- read.csv("~/huitu/FigureYa230immunelandscape/easy_input_type.csv", row.names = 1)
head(type)
...
#library(colorRamp2,lib.loc = "/home/data/vip39/R/x86_64-pc-linux-gnu-library/4.0")
...
subdata2$Cluster <- factor(subdata2$Cluster,levels = c("C1","C2"))
...
plot_cell_trajectory(CDS, color_by = "State", cell_size = 2)+
  scale_colour_manual(values=pal_npg()(12))+
...
CDS1$Cluster[CDS1$Cluster=='C1' & CDS1$State %in% 2] <- '1B'
...
ggsurvplot(fit,
           ...
           conf.int=FALSE,size=1.3,
           pval=T,pval.method = T,
           legend.labs=c("1A","1B","1C","1D","Other"), 
           legend.title="C1",
           xlab="Time (years)",
           ylab='Overall survival')
...
CDS2 <- CDS
CDS2$Cluster[CDS2$Cluster=='C2' & CDS2$State %in% c(10,11)]<- '2A'
CDS2$Cluster[CDS2$Cluster=='C2' & CDS2$State %in% 9] <- '2B'
...
ggsurvplot(fit2,
           ...
           conf.int=FALSE,size=1.3,
           pval=T,pval.method = T,
           legend.labs=c("2A","2B","2C","2D","Other"), 
           legend.title="C2",
           xlab="Time (years)",
           ylab='Overall survival')
...
regulatoryElements <- unique(c(regulatoryElements,c("CEBPB","IRF4","RUNX1","SPI1","STAT2","ZNF683")))
table(regulatoryElements%in%rownames(tcgaBLCA))
...
# 这里量力而行设置多核，或者直接单核运算
options(cluster=snow::makeCluster(spec = 20, "SOCK")) # 打开4核并行计算（不确定是不是4核，不过我windows只用4，服务器我开12）
